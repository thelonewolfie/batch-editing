...
class BatchEditDialog(QDialog):
    ...
    def _setupUi(self):
        ...
        self.cb_find_and_replace = QCheckBox(self)
        self.cb_find_and_replace.setText("Find and Replace")
        self.cb_find_and_replace.setChecked(False)
        ...
        top_hbox.addWidget(self.cb_find_and_replace)

        self.tedit = QPlainTextEdit()
        self.tedit.setTabChangesFocus(True)
        
        t2label = QLabel("New content to replace")
        self.tedit2 = QPlainTextEdit()
        self.tedit2.setTabChangesFocus(True)
        self.tedit2.setDisabled(True)
        
        def disable_text2_toggle():
            self.tedit2.setDisabled(not self.cb_find_and_replace.isChecked())
            
            
        self.cb_find_and_replace.stateChanged.connect(disable_text2_toggle)
        ...
        replace_btn = button_box.addButton("&Replace",
                                           QDialogButtonBox.ActionRole)
        find_and_replace_btn = button_box.addButton("Find &and &Replace",
                                           QDialogButtonBox.ActionRole)
        ...
        find_and_replace_btn.setToolTip("Find all occurences and replace with new in the existing contents")
        ...
        find_and_replace_btn.clicked.connect(
            lambda state, x="findandreplace": self.onConfirm(x))
        ...
        bottom_hbox.addWidget(self.cb_find_and_replace)
        bottom_hbox.addWidget(button_box)

        vbox_main = QVBoxLayout()
        vbox_main.addLayout(top_hbox)
        vbox_main.addWidget(self.tedit)
        vbox_main.addWidget(t2label)
        vbox_main.addWidget(self.tedit2)
    ...
    def onConfirm(self, mode):
        ...
        if mode == "replace":
            ...
            if not askUser(q, parent=self):
                return
        batchEditNotes(browser, mode, nids, fld, text,text2, isHtml=isHtml)
        self.close()


def batchEditNotes(browser, mode, nids, fld, html,text2, isHtml=False):
    if not isHtml:
    ...
    for nid in nids:
        ...
        if fld in note:
            ...
            elif mode == "findandreplace":
               note[fld]=note[fld].replace(html,text2)
